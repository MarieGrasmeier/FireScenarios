...
var mouse_is_free := true
var can_move := false
var can_climb_ladder := false
var introduction := "introduction"
@onready var info_panel = $"../UIManager/CanvasLayer/InfoPanel"
...
func _ready():
	...
	info_panel.connect("panel_visibility_changed", Callable(self, "_on_panel_visibility_changed"))
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("crouch"): # removed the following from the if statement:  or $RayCast.is_colliding()
		if is_on_floor():
		...
		if Input.is_action_pressed("sprint"):
			...
			is_sprinting = true
			is_crouching = false
			if can_climb_ladder == true:
				current_speed = CLIMBING_SPEED
		else:
	...
	if can_climb_ladder:
		velocity.y = direction.z*current_speed
...
func toggle_mouse_mode():
	print("mouse mode toggled")
	print(mouse_is_free)
	if mouse_is_free == true:
		Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
		mouse_is_free = false
		can_move = true
	else:
...
func _on_ga_plan_pressed():
	show()


func _on_fire_plan_pressed():
	pass # Replace with function body.


func _on_notepad_button_toggled(button_pressed):
	pass # Replace with function body.


func _on_bridge_button_pressed():
	toggle_mouse_mode()
...
func _on_dialogue_ab_dialogue_started():
	toggle_mouse_mode()
	



func _on_dialogue_imdg_dialogue_started():
	toggle_mouse_mode()
...
func _on_dialogue_360_completed_dialogue_started():
	toggle_mouse_mode()
...
func _on_dialogue_decision_dialogue_started():
	if mouse_is_free == false:
		toggle_mouse_mode()
